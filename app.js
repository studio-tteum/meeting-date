...
const PEOPLE = [
  /* submitted: 의견 제출 여부. 미제출자는 picks가 없어 합산에 기여하는 게 없다 —
     화면 3 제출 현황 표와 캡션("6명 중 4명")이 이 플래그를 읽는다. */
  { name: '서연', role: 'required', host: true, team: '기획팀', submitted: true,
    busy: [{ h: '1-15', title: '외부 미팅' }, { h: '1-16', title: '외부 미팅' }],
    picks: ['0-9', '4-17'], reasons: ['4-14'], // 금 오후 외부 일정 — 하늘과 겹쳐 검붉어진다
    notes: { '4-14': '외부 일정이 잡힐 수 있음' } },
  { name: '지훈', role: 'required', team: '개발팀', submitted: true, busyDays: [2, 3], busyDayTitle: '외근',
    busy: [{ h: '1-16', title: '팀장 면담' }], // 서연의 외부 미팅(화 15~17시)과 겹치는 16시
    picks: [], reasons: [], notes: {} },
  { name: '민수', role: 'required', team: '디자인팀', submitted: true,
    busy: [{ h: '0-9', title: '주간 보고' }, { h: '0-10', title: '주간 보고' }, { h: '2-14', title: '디자인 리뷰' }],
    picks: ['0-13', '1-13', '4-13', '0-9'], reasons: [] },
  { name: '하늘', role: 'required', team: '마케팅팀', submitted: true,
  ...
  { name: '예은', role: 'optional', team: '개발팀', submitted: false, busy: [{ h: '0-14', title: '면접' }], picks: [], reasons: [] },
  { name: '태윤', role: 'optional', team: '디자인팀', submitted: false, busy: [], picks: [], reasons: [] }
];
...
function computeAgg() {
  ...
  const subN = PEOPLE.filter(p => p.submitted).length;
  document.getElementById('agg-sub').textContent = subN === PEOPLE.length
    ? `참여자 ${PEOPLE.length}명의 의견이 합산되었습니다`
    : `참여자 ${PEOPLE.length}명 중 ${subN}명의 의견이 합산되었습니다`;
...
/* 제출 현황 표(화면 3) — 화면 1 인원 목록과 같은 행 문법의 읽기 전용판.
   아바타 줄(지금 있는 인원) 아래에서 누가 의견을 냈고 누가 대기 중인지 보여준다. */
function renderAggPeople() {
  document.getElementById('agg-people').innerHTML = PEOPLE.map(p => `
    <li class="person${p.role === 'required' ? ' required' : ''}">
      <div class="prow ro">
        ${avatar(p)}
        <span class="pname-group">
          <span class="pname">${p.name}</span>
        ...
        <span class="role-text">${p.role === 'required' ? '필수' : '선택'}</span>
        <span class="sub-badge${p.submitted ? '' : ' wait'}">${p.submitted ? '제출' : '대기 중'}</span>
      </div>
    </li>`).join('');
}
...
function renderAggView() {
  ...
  // 개인 뷰에선 시간 선택이 안 되는 이유를 캡션으로 말한다 — 침묵하면 버그로 읽힌다
  const viewNote = document.getElementById('agg-view-note');
  const person = typeof p3Selected === 'string' ? null : p3Selected;
  viewNote.hidden = !person;
  if (person) viewNote.textContent =
    `${person.name}님의 현황이에요 · 시간 선택은 전체 뷰에서 할 수 있어요`;
